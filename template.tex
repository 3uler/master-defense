...
\item $\mathit{\Theta}^{1},\, \mathit{\Theta}^{2}$ - anti-commuting Majorana spinors
\end{itemize}

\item Fix $\kappa$-symmetry and apply bosonic light-cone gauge \\
$\Rightarrow$ action at most quartic in complex Grassmann fields $\theta^{i}, \eta^{i}$ (inherited from $\mathit{\Theta}$)
...
\only<1>{
generate {\color{blue!90}ensemble} of field configurations $\lbrace \phi_{1},\ldots,\phi_{N}\rbrace$ distributed according $P[\phi]=e^{-S_{\rm E}[\phi]}/Z$\\[3mm]
...
%
\only<2>{
generate {\color{blue!90}ensemble} of field configurations $\lbrace \phi_{1},\ldots,\phi_{N}\rbrace$ distributed according $P[\phi]=e^{-S_{\rm E}[\phi]}/\tilde{Z}\cdot \sqrt{\det \hat{\mathcal{O}}_{\rm F}}$\\[3mm]
{\color{hublue}\bf Ensemble average}
\begin{tcolorbox}[colback=white!95!black, colframe=white!90!black]
\begin{center}
\vspace{-2mm}
...
\path (-1,-1) -- node[left]{\footnotesize $LM=c$} (-1,1);
